...
Блок «Нам доверяют» собран из плиток. Сейчас в плитке только название,
набранное текстом. Когда заказчик пришлёт настоящие логотипы:

1. Положить файлы в `assets/img/clients/` — лучше SVG или PNG с прозрачным фоном,
   светлые: фон блока тёмный.
2. Добавить `<img>` первой строкой внутрь нужного `<li>`:

```html
<li class="trust__i">
  <img src="assets/img/clients/sber.svg" alt="Сбербанк">
  <span>Сбербанк</span>
</li>
```

Название под логотипом остаётся, высота логотипа и отступы подстроятся сами.
Плитки без логотипа продолжат работать как есть, смешивать можно.

Логотипы чужих компаний — их товарные знаки. Перед публикацией стоит убедиться,
что заказчики не против упоминания и показа своих знаков на сайте подрядчика.
...
Пути к стилям и скрипту в обеих страницах заканчиваются на `?v=5`. После правки
`style.css` или `main.js` номер нужно увеличить (`?v=6`) **в обоих файлах**,
